...
if __name__ == '__main__':
    ...
    if args.algorithm == "all":
        for algorithm in _ALL_:
            ...
            np.savetxt("classification/%s/confusion_matrix__%s.out" % (args.collection, algorithm), confusion_matrix,
                       fmt="%d")
    else:
        confusion_matrix = classify_and_predict(X, Y, args.algorithm)
        np.savetxt("classification/%s/confusion_matrix__%s.out" % (args.collection, args.algorithm), confusion_matrix,
                   fmt="%d")
